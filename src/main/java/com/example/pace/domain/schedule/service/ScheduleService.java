...
public class  ScheduleService {
    ...
    // 일정 수정
    @Transactional
    public ScheduleResDto updateSchedule(Long memberId, Long  scheduleId, ScheduleUpdateReqDto request, UpdateScope scope ) {
        Schedule schedule = scheduleRepository.findByMemberIdAndId(memberId, scheduleId)
        ...
        if (UpdateScope.ALL.equals(scope) && schedule.getRepeatGroupId() != null) {
            ...
            if (request.getRepeatInfo() != null && isRepeatRuleChanged(schedule.getRepeatRule(), request.getRepeatInfo())) {
                LocalDate currentStart = (request.getStartDate() != null) ? request.getStartDate() : schedule.getStartDate();
